...
int main(int argc,char*argv[]){
  ...
  int cannylow = atoi(argv[6]);
  int cannyhigh = atoi(argv[7]);
  int mode = atoi(argv[8]);
  EdgeMap *map = NULL;
  Timer timer;
    
  printf("ED in.pgm out.pgm gradtresh anchortresh sigma cannylow cannyhigh mode\n");
  printf("mode 0: ED\n");
  printf("mode 1: EDPF\n");
  printf("mode 2: CannySR\n");
  printf("mode 3: CannySRPF\n");
  ...
  if (mode == 0) {
  timer.Start();
  map = DetectEdgesByED(srcImg, width, height, SOBEL_OPERATOR, gradtresh, anchortresh, sigma);
  timer.Stop();
  ...
  if (mode == 1) {
  timer.Start();
  map = DetectEdgesByEDPF(srcImg, width, height, sigma);
  timer.Stop();
  ...
  if (mode == 2) {
  timer.Start();
  map = DetectEdgesByCannySR(srcImg, width, height, cannylow, cannyhigh, 3, sigma);
  timer.Stop();
  ...
  if (mode == 3) {
  timer.Start();
  map = DetectEdgesByCannySRPF(srcImg, width, height, 3, sigma);
  timer.Stop();
