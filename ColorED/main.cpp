...
int main(int argc,char*argv[]){
  ...
  int anchortresh = atoi(argv[4]);
  int sigma = atoi(argv[5]);
  int mode = atoi(argv[6]);

  printf("ColorED in.ppm out.pgm gradtresh anchortresh sigma mode\n");
  
  if (ReadImagePPM(str, &width, &height, &redImg, &greenImg, &blueImg) == false) exit;

  timer.Start();
  
  if (mode == 0) {
  map = ColorED(redImg, greenImg, blueImg, width, height, gradtresh, anchortresh, sigma);
  printf("mode 0: ColorED\n");}   // Color edge detection by ColorED
  if (mode == 1) {
  map = ColorEDV(redImg, greenImg, blueImg, width, height, gradtresh, sigma);
  printf("mode 1: ColorEDV\n");}   // Color edge detection by ColorED
  if (mode == 2) {
  map = ColorEDPF(redImg, greenImg, blueImg, width, height, sigma);
  printf("mode 2: ColorEDPF\n");}
  timer.Stop();

  printf("ColorED returns %3d edge segments for image and takes %5.2lf ms\n", map->noSegments,   timer.ElapsedTime());
